...
if [ -f ${IMAGE}/DockerfileXNAT ]; then
	echo "Removing ${IMAGE}/DockerfileXNAT"
	rm ${IMAGE}/DockerfileXNAT
fi

echo "## This file is autogenerated by build_docker.sh. Manual changes are OVERWRITTEN"; cat  ${IMAGE}/Dockerfile >> ${IMAGE}/DockerfileXNAT
python command2label.py ${IMAGE}/command.json >> ${IMAGE}/DockerfileXNAT

IMAGE_SPEC="${DOCKER_REPO}/${IMAGE}:${TAG}"
docker build -f DockerfileXNAT -t ${IMAGE_SPEC} .
